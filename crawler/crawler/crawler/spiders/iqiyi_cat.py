...
class iqiyi_cat(Spider):
    ...
    def __init__(self, *args, **kwargs):
        super(iqiyi_cat, self).__init__(*args, **kwargs)
        self._cat_urls = []
        '''
        ...
        '''
        try:
            self._cat_urls = self.mgr.get_cat_url('iqiyi')
    ...
    def start_requests(self):
        try:
            ...
            for cat in self._cat_urls:
                items.extend([Request(url=cat['url'], callback=self.parse_page,meta={'page': 1, 'cat_id': cat['id'], 'cat_name': cat['cat_name'],'audit':cat['audit'],'priority':cat['priority']})])
    ...
    def parse_page(self, response):
        try:
            ...
            for v in qy_v:
                ...
                items.append(Request(url=url[0].strip(), callback=self.parse_episode, meta={'cat_id': cat_id, 'cat_name': cat_name, 'thumb': thumb,'audit':audit,'priority':priority}))
